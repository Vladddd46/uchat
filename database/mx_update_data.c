...
static char* mx_create_request(char* where, char* id, char* what, char* val) {
    char* request = mx_strjoin("UPDATE DATA SET ", what);

    request = mx_strjoin(request, " = '");
    request = mx_strjoin(request, val);
    request = mx_strjoin(request, "' WHERE ");
    request = mx_strjoin(request, where);
    request = mx_strjoin(request, " = '");
    request = mx_strjoin(request, id);
    request = mx_strjoin(request, "'");
    // printf("%s", request);
    return request;
}
...
int mx_update_data(char* str, char* where, char* id, char* what, char* val) {
    ...
    char *sql = mx_create_request(where, id, what, val); 
