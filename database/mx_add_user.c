...
static char* mx_insert_all_args(char* login, char* password) {
	char* request = mx_strjoin("INSERT INTO USERS (LOGIN, PASSWORD) VALUES('", login);
...
int mx_add_user(const char *str, char* login, char* password) {
    ...
    char* sql = mx_insert_all_args(login, password);
